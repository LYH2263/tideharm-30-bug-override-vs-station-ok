...
onMounted(async () => {
  try {
    ...
    const d = await getJSON<{ threshold_m: number; threshold_source: string; max_abs_residual_m: number; items: Row[] }>(`/api/stations/${slug}/residuals`)
    thr.value = d.threshold_m
    thrSource.value = d.threshold_source
...
<template>
  <div class="page">
    <h1>残差表</h1>
    <p class="lead">阈值 {{ thr }} m（{{ thrSource === 'override' ? '站级覆盖' : '全局默认' }}） · 最大 |残差| {{ maxAbs }} m ·
